...
mapfile -t SALESFORCE_DEPLOYLOG < <$(sf project deploy start --source-dir "force-app/main/default/classes/CreatingAccount1.cls" --source-dir "force-app/main/default/classes/CreatingAccount1.cls-meta.xml" --dry-run --test-level NoTestRun --target-org ${SALESFORCE_ORG_ALIAS})

echo "--------- DEPLOY TEST ------------"
echo $TEST
echo ARRAY_LEN=${#SALESFORCE_DEPLOYLOG[@]}

echo "--------- ARRAY TEST ---------"
echo $ARRAY_LEN
echo "--------- ARRAY ITEM TEST ---------"
echo ${SALESFORCE_DEPLOYLOG[0]}
